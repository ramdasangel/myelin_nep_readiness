#!/usr/bin/env python3
"""
Stage-1 Teacher Intent — FP1 to FP5 Distribution Dashboard
setCodes: 1234 (English) + 103 (Marathi)
Shows raw FP response counts per question per branch.
"""
...
OUT_PATH = os.path.join(BASE, "output", "dashboards", "12a_stage1_teacher_fp.html")
...
def load_data():
    # Raw: every response row → one FP vote
    # Per branch: {fp: total_votes}
    # Per branch per user: track for user count
    ...
    # Per-response FP counts (raw, not per-user)
    overall_fp = {fp: 0 for fp in FPS}
    total_responses = 0

    # Per user: role, branch, fp_votes, opt_values per FP
    users = {}
        ...
        with open(fpath) as f:
            for row in csv.DictReader(f):
                ...
                if not bc or bc.startswith("m") or not uid:
                    continue
                # Only include teachers (role from CSV)
                if role != "Teacher":
                    continue
                fp = GOAL_FP.get(goal)
                if not fp:
                ...
                if uid not in users:
                    users[uid] = {"bc": bc, "lang": lang, "fp_votes": defaultdict(int), "fp_opts": defaultdict(list)}
                users[uid]["fp_votes"][fp] += 1
                try:
                    users[uid]["fp_opts"][fp].append(int(row.get("selectedOption", "")))
                except (ValueError, TypeError):
                    pass

    return branch_fp_votes, branch_users, branch_lang, overall_fp, total_responses, users


def build_html(branch_fp_votes, branch_users, branch_lang, overall_fp, total_responses, users, branch_names):
    j = json.dumps
    ...
    for bc in all_bcs:
        ...
        total_votes = sum(branch_fp_votes[bc].values())
        ...
        cells += f'<td style="text-align:center">{n_users}</td>'
    ...
    overall_pcts = j([round(overall_fp[fp] / total_responses * 100, 1) if total_responses > 0 else 0 for fp in FPS])

    # Stacked bar per branch (counts)
    stacked = j([{
    ...
    # Per-branch radar data (JSON for JS)
    branch_radar = {}
    for bc in all_bcs:
        total = sum(branch_fp_votes[bc].values()) or 1
        branch_radar[bc] = [round(branch_fp_votes[bc][fp] / total * 100, 1) for fp in FPS]

    # Pre-build strings that have quoting issues inside f-strings
    fp_th_branch = "".join(
        '<th style="text-align:center;color:%s" onclick="sortTbl(%d,&quot;n&quot;)">%s</th>' % (FP_COLORS[i], 4+i, fp)
        for i, fp in enumerate(FPS)
    )
...
<title>Stage-1 Teacher Intent — FP Distribution</title>
...
<div class="header">
<div><h1>Stage-1 Teacher Intent — FP1 to FP5</h1>
<div class="sub">Raw Response Counts | setCodes: 1234 (EN) + 103 (MR) | Teachers Only</div></div>
...
<div class="cards">
<div class="card"><div class="label">Teachers</div><div class="value" style="color:var(--blue)">{total_users}</div><div class="sub">{total_en} English + {total_mr} Marathi</div></div>
<div class="card"><div class="label">Branches</div><div class="value" style="color:var(--teal)">{len(all_bcs)}</div><div class="sub">with teacher intent data</div></div>
...
{"".join(f'<div class="card" style="border-left:4px solid {FP_COLORS[i]}"><div class="label">{fp}: {FP_NAMES[fp]}</div><div class="value" style="color:{FP_COLORS[i]}">{overall_fp[fp]:,}</div><div class="sub">{overall_fp[fp]/total_responses*100:.1f}% of all responses</div></div>' for i, fp in enumerate(FPS))}
</div>

<div class="info-box">
<strong>Raw FP counts:</strong> Each teacher answers ~22 questions. Each answer's selected option is tagged with one FP (FP1-FP5).
These are raw counts of how many times each FP was selected across all responses — NOT per-user dominant FP.
Note: Teacher instrument has 5 Qs for FP3/FP4 vs 4 Qs for FP1/FP2/FP5.
<a href="./12b_stage1_leader_fp.html" style="font-weight:600">View Leader Dashboard →</a>
</div>
...
<div class="chart-box"><h3>FP Distribution by Branch (stacked counts)</h3><canvas id="stackedChart" height="500"></canvas></div>
<div class="chart-box"><h3>FP Distribution by Branch (% of branch total)</h3><canvas id="pctChart" height="500"></canvas></div>
</div>

<!-- BRANCHES -->
<div id="branches" class="content">
<h2 class="section-title">Branch Detail — Raw FP Response Counts</h2>
<div class="tbl-wrap"><table id="branchTable">
<thead><tr>
<th onclick="sortTbl(0,'s')">Branch</th><th>Name</th><th style="text-align:center" onclick="sortTbl(2,'n')">Users</th><th style="text-align:center">Lang</th>
{fp_th_branch}
</tr></thead>
...
<div style="text-align:center;padding:20px;color:var(--muted);font-size:11px">Stage-1 Teacher Intent | Project Kshitij | Myelin | {date.today()}</div>
...
def main():
    print("Loading Stage-1 Teacher intent data...")
    branch_fp, branch_users, branch_lang, overall_fp, total_resp, users = load_data()
    branch_names = load_branch_names()
    total_users = sum(len(v) for v in branch_users.values())
    print(f"  {total_users} teachers, {len(branch_fp)} branches, {total_resp:,} responses")
    html = build_html(branch_fp, branch_users, branch_lang, overall_fp, total_resp, users, branch_names)
