#!/usr/bin/env python3
"""
Stage-1 Leader Intent — ADV (Aspirational Depth Vector) Dashboard
...
def load_data():
    ...
    return (branch_adv, branch_users, branch_lang, branch_source, overall_adv,
            branch_fp_raw, overall_fp_raw, total_raw, users, user_adv)


def build_html(branch_adv, branch_users, branch_lang, branch_source, overall_adv,
    ...
    # Build table headers
    fp_th_profile = "".join(
        '<th style="text-align:center;color:%s" onclick="sortTbl(%d,&quot;n&quot;,&quot;profileTable&quot;)">%s ADV</th>' % (FP_COLORS[i], 5 + i, fp)
        for i, fp in enumerate(FPS)
    )
...
<title>Stage-1 Leader Intent — ADV Dashboard</title>
...
<div class="header">
<div><h1>Stage-1 Leader Intent — Aspirational Depth (ADV)</h1>
<div class="sub">ADV = mean(selectedOption) / 3 per FP section | Leader instrument (7890/104) + Leaders from teacher instrument (1234/103)</div></div>
...
<div class="card"><div class="label">Highest ADV</div><div class="value" style="color:var(--green)">%(best_fp)s</div><div class="sub">%(best_fp_adv).3f across all leaders</div></div>
%(fp_cards)s
</div>

<div class="info-box">
<strong>Aspirational Depth Vector (ADV):</strong> For each FP section, ADV = mean(selectedOption) / 3.
This ranges from 0 (shallowest) to 1 (deepest aspiration). The Source column shows L (from leader instrument 7890/104) or T (leaders who took teacher survey 1234/103).
...
These proportions are fixed per user. Use the ADV metric (Overview / Branch Profile tabs) for meaningful comparisons.
</div>
...
<div style="text-align:center;padding:20px;color:var(--muted);font-size:11px">Stage-1 Leader Intent (ADV) | Project Kshitij | Myelin | %(today)s</div>
...
def main():
    print("Loading Stage-1 Leader intent data (ADV)...")
    (branch_adv, branch_users, branch_lang, branch_source, overall_adv,
    ...
    print("  %d branches" % len(branch_adv))
    for fp in FPS:
        print("  %s ADV = %.3f" % (fp, overall_adv[fp]))
    html = build_html(branch_adv, branch_users, branch_lang, branch_source, overall_adv,
                      branch_fp_raw, overall_fp_raw, total_raw, users, user_adv, branch_names)
